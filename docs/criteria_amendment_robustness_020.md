...
**Filed 14 September 2026. Checklist re-signed 15 September 2026.**

**This amendment was made AFTER the results were known.**
...
| | |
|---|---|
| Checklist re-derived and re-signed | 15 September 2026 |
| New checklist SHA-256 | `fa98c1fd149c2ad7…` (was `8e004aa60605bb9f…`) |
| Signer | Prasad Devendra Watane, on his instruction |
| First model response in the log | 11 August 2026 |

The signature therefore postdates the evidence by five weeks. The report no
longer claims pre-registration for this checklist: its header states that the
checklist was signed after responses were collected, and each robustness finding
carries the revised and original verdicts on its own line. Both statements are
generated by code rather than written by hand, so neither can be dropped by
editing prose.

Probe regeneration reproduced all 14,825 probes with coverage 1.00 against the
existing 61,259 responses, so no model output was invalidated by the re-sign.
